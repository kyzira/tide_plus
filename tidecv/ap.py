...
class APDataObject:
	...
	def get_precision(self) -> float:
		if len(self.data_points) == 0:
			return 0
		true_pos = sum(1 for _, is_true, _ in self.data_points.values() if is_true)
		return true_pos / len(self.data_points)

	def get_recall(self) -> float:
		if self.num_gt_positives == 0:
			return 0
		true_pos = sum(1 for _, is_true, _ in self.data_points.values() if is_true)
		return true_pos / self.num_gt_positives
...
class ClassedAPDataObject:
	...
	def get_precision(self) -> float:
		precisions = [x.get_precision() for x in self.objs.values() if not x.is_empty()]
		return sum(precisions) / len(precisions) if len(precisions) > 0 else 0

	def get_recall(self) -> float:
		recalls = [x.get_recall() for x in self.objs.values() if not x.is_empty()]
